"""Tool to run the rapamycin model factory and simulation experiments."""
...
from pkdb_models.models.rapamycin import RAPAMYCIN_PATH
from pkdb_models.models.rapamycin.simulations import run_simulation_experiments, EXPERIMENTS
from sbmlutils.console import console

FACTORY_SCRIPT_PATH = RAPAMYCIN_PATH / "models" / "factory.py"
...
def _setup_custom_results_paths(results_dir: str):
    ...
    rapamycin.RESULTS_PATH = custom_path
    rapamycin.RESULTS_PATH_SIMULATION = custom_path / "simulation"
...
def _get_current_results_path():
    ...
    from pkdb_models.models.rapamycin import RESULTS_PATH
    return RESULTS_PATH
...
def _list_available_experiments():
    ...
    console.print('[dim]Example: run_rapamycin --action simulate --experiments "misc,Anderson1961"[/dim]')
    console.print('[dim]Or use "all" to run all experiments: run_rapamycin --action simulate --experiments all[/dim]\n')
...
def main() -> None:
    ...
    console.rule("[bold cyan]RAPAMYCIN PBPK MODEL[/bold cyan]", style="cyan")
    ...
    if options.results_dir:
        ...
        if options.action and options.action.lower() == "factory":
            console.print(f"[cyan]Factory will use model base path: {rapamycin.MODEL_BASE_PATH}[/cyan]")
        else:
...
if __name__ == "__main__":
    """
    This script is intended to be run from the command line as 'run_rapamycin'.
    ...
    2. Run the Model Factory:
       ...
       $ run_rapamycin --action factory

    3. Run Simulations:
       List available experiments:
       $ run_rapamycin --action list_experiments

       Run all experiments:
       $ run_rapamycin --action simulate --experiments all

    5. Run Everything:
       Runs factory and all simulations.
       $ run_rapamycin --action all
       
       With custom results directory for figures:
       $ run_rapamycin --action all --results-dir '/path/to/my/results'
    """
    main()