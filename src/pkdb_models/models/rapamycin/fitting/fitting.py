...
from pkdb_models.models.rapamycin.fitting.fit_experiments import (
    f_fitexp_control,
)
from pkdb_models.models.rapamycin.fitting.parameters import (
    parameters_all,
)

from pkdb_models.models.rapamycin import (
    RAPAMYCIN_PATH,
    DATA_PATHS,
)
...
def get_fit_experiments(fit_subset: FitExperimentSubset, study_ids: List[str] = None):
    ...
    if fit_subset == FitExperimentSubset.CONTROL:
...
def main() -> None:
    ...
    The script is registered as `fit_rapamycin` command.
    """
    ...
    console.print(":wrench: FIT RAPAMYCIN :wrench:")
    console.rule(style="white")
    ...
    if not options.output_dir:
        from pkdb_models.models.rapamycin import RESULTS_PATH_FIT
    ...
    results: Dict[str, Tuple[OptimizationResult, OptimizationProblem]] = fit_rapamycin(
        fit_experiments=fit_experiments,
...
if __name__ == "__main__":
    ...
    fit_rapamycin
    fit_rapamycin --cores=10 --runs=10 --seed=1234 --method=LSQ --strategy=ALL --subset=CONTROL --name=RAPAMYCIN_LSQ_CONTROL
    """
    main()